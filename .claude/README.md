...
```
.claude/
├── CLAUDE.md                       # 세션 규칙 — 프로젝트 컨텍스트 + 불변식 INV-1~11 + Rule #1~#14
...
이 프로젝트의 사고는 문법 오류가 아니라 **도메인 규칙 위반**에서 발생한다. 오버부킹, 잘못된 가격 청구, 타 테넌트 데이터 노출, 채널의 상품 자동 비활성화. 그래서 하네스의 중심은 린터가 아니라 **불변식 11개**다.
...
`verify_hook.mjs` 가 파일 저장 시점에 이 중 정적 검출 가능한 항목(INV-1·5·8·10 + 시크릿 + SQLi + 루프 우회)을 **자동 차단**한다.
...
훅이 실제로 트리거를 잡고 불변식 위반을 차단하는지 8개 케이스로 검증한다. 하네스 파일을 수정한 뒤에는 반드시 실행할 것.
