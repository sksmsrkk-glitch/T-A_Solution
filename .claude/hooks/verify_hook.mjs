...
function checkSql(text, file) {
  ...
  if (hasTenantColumn && !hasRls) {
    ...
    WARN.push(
      `  [INV-5] RLS 는 켰지만 정책(create policy)이 없습니다 — 전체 접근 차단 상태입니다.`,
    );
  }

  if (
    /\bnumeric\b|\bdecimal\b|\bdouble\s+precision\b|\breal\b/.test(lower) &&
    /amount|price|fare|fee|net|total/.test(lower)
  ) {
    WARN.push(
      `  [INV-10] 금액 컬럼에 부동소수/numeric 타입이 보입니다 — integer minor unit + currency 를 사용하세요.`,
    );
  }

  if (/timestamptz/.test(lower) && /slot|availability/.test(lower)) {
    WARN.push(
      `  [INV-8] 슬롯 시각에 timestamptz 가 보입니다 — local_date/local_start_time + timezone 을 사용하세요.`,
    );
  }

  if (/user_metadata/.test(lower)) {
    BLOCK.push(
      `  [INV-5] RLS 정책에 user_metadata 사용은 금지입니다(클라이언트 조작 가능). app_metadata 를 사용하세요.`,
    );
  }

  if (/drop\s+(table|column)/.test(lower)) {
    WARN.push(
      `  [MIGRATION] 파괴적 변경(DROP)이 포함되어 있습니다 — 사용자 사전 승인이 필요합니다.`,
    );
  }
}
...
function checkTs(text, file) {
  ...
  add(
    BLOCK,
    text,
    /NEXT_PUBLIC_[A-Z_]*(SERVICE_ROLE|SECRET|PRIVATE|SERVICE_KEY)/,
    'SECRET',
    'NEXT_PUBLIC_* 환경변수에 비밀값이 담겨 있습니다 — 클라이언트 번들에 노출됩니다.',
  );
  add(
    BLOCK,
    text,
    /(eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,})|(sb_secret_[A-Za-z0-9]{10,})/,
    'SECRET',
    '하드코딩된 토큰/키로 보이는 문자열이 있습니다 — 환경변수로 옮기세요.',
  );

  const isClientFile =
    /[\\/](app|components|pages|hooks)[\\/]/.test(file) || /^'use client'/m.test(text);
  if (isClientFile && /service_role|SERVICE_ROLE_KEY/.test(text)) {
    BLOCK.push(
      `  [SECRET] 클라이언트 코드에서 service_role 키를 참조하고 있습니다 — 서버 전용입니다.`,
    );
  }
  ...
  add(
    ...
    /@ts-ignore/,
    'LOOP',
    '@ts-ignore 로 타입 오류를 덮는 것은 금지입니다 (loop_agent forbidden_fixes).',
  );
  add(
    WARN,
    text,
    ...
    'LOOP',
    '파일 단위 eslint-disable 이 있습니다 — 규칙을 끄지 말고 원인을 수정하세요.',
  );
  add(
    WARN,
    text,
    ...
    'TYPE',
    '이중 단언(as unknown as)은 타입 은폐 신호입니다.',
  );
  add(
    WARN,
    text,
    /:\s*any\b|<any>/,
    'TYPE',
    '`any` 사용이 감지되었습니다 — strict 정책상 금지입니다.',
  );
  add(
    WARN,
    text,
    /\.(skip|todo)\s*\(|xit\s*\(|xdescribe\s*\(/,
    'LOOP',
    '테스트 skip 이 있습니다 — 통과를 위한 우회인지 확인하세요.',
  );

  // 재고 안전성 (INV-1)
  if (/capacity\s*\?\?\s*0|capacity\s*\|\|\s*0/.test(text)) {
    BLOCK.push(
      `  [INV-1] capacity 의 NULL 은 "무제한(FREESALE)"을 의미합니다. \`?? 0\` 로 기본값을 넣으면 무한재고 상품이 즉시 매진됩니다.`,
    );
  }
  if (/held_count\s*=\s*held_count\s*\+/.test(text) && !/capacity\s+is\s+null/i.test(text)) {
    WARN.push(
      `  [INV-1] 재고 차감 UPDATE 에 \`capacity IS NULL\` 분기가 보이지 않습니다 — FREESALE 상품이 매진 처리될 수 있습니다.`,
    );
  }
  ...
  add(
    BLOCK,
    text,
    /replicaDb\s*(\.\w+)*\s*\.\s*(insert|update|delete)\s*\(/,
    'INV-12',
    '읽기 복제본에 쓰기를 시도하고 있습니다 — 복제본은 SELECT 전용입니다. primaryDb 를 사용하세요.',
  );
  if (
    /replicaDb/.test(text) &&
    /(held_count|booked_count|BookingState|price_snapshot)/.test(text)
  ) {
    WARN.push(
      `  [INV-12] 재고·예약 상태 코드에서 replicaDb 참조가 보입니다 — 복제 지연으로 오버부킹이 발생할 수 있습니다. 차감·전이·확정 판정은 primaryDb 여야 합니다.`,
    );
  }
  ...
  if (/for\s*\(|\.forEach\s*\(/.test(text) && /await\s+(primaryDb|replicaDb|db)\s*\./.test(text)) {
    WARN.push(
      `  [PERF] 반복문 안에서 쿼리를 await 하고 있을 수 있습니다 (N+1) — 배치 조회로 대체하세요.`,
    );
  }
}
